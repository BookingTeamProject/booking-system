...
const formatAvatar = (url?: string | null): string => {
  if (!url) return 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=120&q=80';
...
export const Navbar: React.FC = () => {
  ...
  const isActive = (p: string) => location.pathname === p || (p === '/menu' && location.pathname.startsWith('/menu'));
    ...
    <header style={headerStyle}>
      <div style={containerStyle}>
        ...
        <nav style={navLinksStyle}>
          <Link to="/" style={isActive('/') ? activeNavStyle : navStyle}>
            🏠 Головна
          </Link>
          <Link to="/menu" style={isActive('/menu') ? activeNavStyle : navStyle}>
            ☰ Меню
          </Link>
          <Link to="/about" style={isActive('/about') ? activeNavStyle : navStyle}>
            👥 Про нас
          </Link>
          <Link to="/promotions" style={isActive('/promotions') ? activeNavStyle : navStyle}>
            🏷️ Акції
          </Link>
          <Link to="/faq" style={isActive('/faq') ? activeNavStyle : navStyle}>
            🎧 Підтримка
          </Link>
        </nav>

        {/* Кнопка регистрации жилья, язык и профиль */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
          {isLandlord ? (
            ...
            <Link to="/menu?tab=properties" style={ctaStyle}>
          ...
          {user ? (
            <div style={userCardWrapperStyle}>
              <Link to="/profile" style={userCardNavStyle}>
                <img
                  src={formatAvatar(user.avatarUrl)}
                  alt="avatar"
                  style={avatarNavStyle}
                  onError={(e) => {
          ...
          <button onClick={() => setMobileNavOpen(!mobileNavOpen)} style={burgerBtnStyle}>
            {mobileNavOpen ? '✕' : '☰'}
          </button>
        </div>
      </div>
      ...
      {mobileNavOpen && (
        <div style={mobileNavStyle}>
          <Link to="/" style={mobileLinkStyle} onClick={() => setMobileNavOpen(false)}>🏠 Головна</Link>
          <Link to="/menu" style={mobileLinkStyle} onClick={() => setMobileNavOpen(false)}>☰ Меню кабінету</Link>
          <Link to="/routes" style={mobileLinkStyle} onClick={() => setMobileNavOpen(false)}>🗺️ Каталог житла</Link>
          <Link to="/about" style={mobileLinkStyle} onClick={() => setMobileNavOpen(false)}>👥 Про нас</Link>
          <Link to="/promotions" style={mobileLinkStyle} onClick={() => setMobileNavOpen(false)}>🏷️ Акції</Link>
          <Link to="/faq" style={mobileLinkStyle} onClick={() => setMobileNavOpen(false)}>🎧 Підтримка</Link>
          <Link to="/profile" style={mobileLinkStyle} onClick={() => setMobileNavOpen(false)}>👤 Профіль</Link>
        </div>
      )}
    </header>
  );
...
// Стили
const headerStyle: React.CSSProperties = { backgroundColor: '#6E473B', position: 'sticky', top: 0, zIndex: 1000, boxShadow: '0 4px 18px rgba(41,28,14,0.15)' };
...
const userCardNavStyle: React.CSSProperties = { display: 'flex', alignItems: 'center', gap: '8px', textDecoration: 'none' };
const avatarNavStyle: React.CSSProperties = { width: '34px', height: '34px', borderRadius: '50%', objectFit: 'cover', border: '1.5px solid #DC9666' };
